...
import (
	"fmt"
	"github.com/aliforever/go-generic-sync-map"
	"math/rand"
	"reflect"
	"runtime"
	internalSync "sync"
...
// }
//
// func applyMap[T any](calls []mapCall) ([]mapResult, map[any]any) {
// 	return applyCalls(new(sync.Map[T]), calls)
// }
...
func TestConcurrentRange(t *testing.T) {
	...
	m := new(sync.Map[int64])
	for n := int64(1); n <= mapSize; n++ {
		m.Store(n, int64(n))
	}

	done := make(chan struct{})
	var wg internalSync.WaitGroup
	defer func() {
		close(done)
		wg.Wait()
	}()
	...
	for n := iters; n > 0; n-- {
		...
		m.Range(func(ki any, vi int64) bool {
			k, v := ki.(int64), vi
			if v%k != 0 {
